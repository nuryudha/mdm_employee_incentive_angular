...
@Component({
  selector: 'app-auth-check-to-do-list',
  templateUrl: './auth-check-to-do-list.component.html',
  styleUrls: ['./auth-check-to-do-list.component.css']
})
export class AuthCheckToDoListComponent implements OnInit {
  ...
  getDetailUser() {
    ...
    this.services.detailUser('details', this.httpOptions, catchError(this.handleError.handleErrorDetailUser.bind(this))).subscribe( result => {
      ...
      if(result.body.status == true) {
        ...
        this.router.navigate(['/to-do-list-auth']);

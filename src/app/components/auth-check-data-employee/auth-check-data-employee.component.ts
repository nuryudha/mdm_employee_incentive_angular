...
@Component({
  selector: 'app-auth-check-data-employee',
  templateUrl: './auth-check-data-employee.component.html',
  styleUrls: ['./auth-check-data-employee.component.css']
})
export class AuthCheckDataEmployeeComponent implements OnInit {
  ...
  getDetailUser() {
    ...
    this.services.detailUser('details', this.httpOptions, catchError(this.handleError.handleErrorDetailUser.bind(this))).subscribe( result => {
      ...
      if(result.body.status == true) {
        ...
        this.router.navigate(['/data-employee-auth']);
